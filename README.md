...
```mermaid
graph TD
    User[用户输入] --> API[FastAPI /chat]
    API --> Graph[LangGraph Agent]
    Graph --> Router{意图识别 Router}
    
    Router -- SQL_QUERY --> SQL[SQL Node]
    Router -- TASK_CREATE --> Task[Task Node]
...
curl -X POST "http://localhost:8000/chat" \
     ...
     -d '{"query": "帮我查一下上个月的销售额", "thread_id": "test-001"}'
```
